...
public:
  ...
  void setLeftMotorPWM(int pwm);   // sets motors 1 and 2 speeds to specified pwm
  void setRightMotorPWM(int pwm);  // sets motors 3 and 4 speeds to specified pwm
  float readDistanceSensor(int sensorId);  // reads and returns distance in cm from specified ultrasonic distance sensor (only valid for sensorIds 1 or 7)

  // Functions only available on physical tanks
  void setMotorPWM(int motorId, int pwm);  // sets the speed of the specifed motor to the specified pwm (only valid for motorIds 1 through 4)
